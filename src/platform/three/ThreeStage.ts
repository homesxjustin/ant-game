...
export class ThreeStage {
  ...
  constructor(canvas: HTMLCanvasElement, worldW: number, worldH: number, terrain: Terrain) {
    ...
    this.renderer.shadowMap.enabled = false; // kept off for headless/perf parity

    this.scene = new THREE.Scene();
    this.scene.fog = new THREE.Fog(0x8ec5e6, 400, 2600);
    ...
    // Lighting: hemisphere for soft sky/ground bounce, a sun for direction and
    // long shadows-of-colour, and a low ambient floor so nothing is pure black.
    this.hemi = new THREE.HemisphereLight(0xbfd8ff, 0x4a5a2a, 0.9);
    this.scene.add(this.hemi);
    this.ambient = new THREE.AmbientLight(0xffffff, 0.25);
    this.scene.add(this.ambient);
    this.sun = new THREE.DirectionalLight(0xfff2cc, 1.1);
